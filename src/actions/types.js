export const FACEBOOK_LOGIN_SUCCESS = 'facebook_login_success'
export const FACEBOOK_LOGIN_FAIL = 'facebook_login_fail'

export const GOOGLE_LOGIN_SUCCESS = 'google_login_success'
export const GOOGLE_LOGIN_FAIL = 'google_login_fail'

export const LOCAL_LOGIN_SUCCESS = 'local_login_success';
export const LOCAL_LOGIN_FAIL = 'local_login_fail';
export const LOCAL_PASSWORD_RESET = 'local_password_reset';

export const NEW_EMAIL_CHANGED = 'new_email_changed';
export const EMAIL_CHANGED = 'email_changed';
export const PASSWORD_CHANGED = 'password_changed';
export const BUTTON_SPINNER = 'button_spinner';
export const SCREEN_SPINNER = 'screen_spinner';
export const IS_USER_LOGGED_IN = 'is_user_logged_in';
export const CLEAR_STATE = 'clear_state';

export const DELETE_ACCOUNT_ERROR = 'delete_account_error';

export const UPDATED_EMAIL_SUCCESS = 'update_email_success';
export const UPDATED_EMAIL_ERROR = 'update_email_error';

export const DETAIL_URL_CHANGED = 'detail_url_changed';
export const AMAZON_PRICE_CHANGED = 'amazon_price_changed';
export const THIRD_PARTY_PRICE_NEW_CHANGED = 'third_party_price_new_changed';
export const THIRD_PARTY_PRICE_USED_CHANGED = 'third_party_price_used_changed';

export const TRACK_ITEM_SUCCESS = 'track_item_success';
export const TRACK_ITEM_ERROR = 'track_item_error';
